# Cookbook written by Amir Szitenberg (https://github.com/HullUni-bioinformatics/TE-search-tools)
# Modified by Damilola Oresegun
# coding: utf-8
...
# <pre>
# run_LTRharvest('coded_genome_assembly', 
#                'name_of_intermideate_file', 
#                'ltrharvest_output')                          
# 
# run_TransposonPSI('coded_genome_assembly',
...
def run_repeat_masker(query, RepeatMasker = '/path/to/conda/envs/and/bin/RepeatMasker', engine=False, parallel=32,
             quick=False, rush=False, nolow=False, noint=False, norna=True, alu=False, div=False,
             ...
             small=False, xsmall=True, poly=False, source=False, html=True, ace=False, gff=True, s=True, u=False,
             xm=False, no_id=True, excln=True, noisy=False, dir=True):
    ...
    args_wo_values = ['s','quick','rush','nolow','noint','norna','alu','is_only','is_clip','no_is'
...
def make_repeatmodeler_database(name, input_filename, BuildDatabase='/path/to/conda/envs/and/bin/BuildDatabase',
...
def run_repeatmodeler(database, RepeatModeler='/path/to/conda/envs/and/bin/RepeatModeler', pa='32', engine='ncbi',
                     species='eukaryota'):
    frame = inspect.currentframe()
    args, _, _, values = inspect.getargvalues(frame)
    del values['frame']
...
def print_online_censor(censor_classifications, filename):
    ...
    with open(filename, 'wb') as csvfile:
        ...
        linewriter.writerow(['Name','class','score'])
...
def run_OneCodeToFindThemAll(pooled_RM_outputs_dir, #The directory containing repeatmasker .out files or a path to a specific .out file
                            ...
                            unknown=False,
                            strict=True,
                            build_dictionary = '/path/to/Tools/OCTFTA/build_dictionary.pl',
                            octfta = '/path/to/Tools/OCTFTA/one_code_to_find_them_all.pl',
                            ):
...
def run_TransposonPSI(input_filename,
                  TPSI = '/path/to/conda/where/transposonPSI/is/installed/bin/transposonPSI.pl'):
        ...
        for e in TEs['taken']:
            ...
            if program == 'TransposonPSI':
                ...
                if max_RM_OC_score:
					...
					score = max([str(s) for s in score.split('/')])
